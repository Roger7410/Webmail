 ...
 */
public class FolderPage extends MailPage {
    public FolderPage(HttpServletRequest request, HttpServletResponse response) {
    ...
    public ST body() {
        ...
        ST st = templates.getInstanceOf("folder");
        HttpSession session = request.getSession();
        String account = (String)session.getAttribute("account");
        Folder folders[] = SQLManager.getFoldersInfoSQL(account);
        st.add("username", getUsername());
        st.add("ea_address", getAccount());
        st.add("infolder", infolder);
        st.add("folder", folders);
